...
from poja.bucket import set_no_bucket
...
from poja.eventstack import set_event_stack
from poja.database.postgres import set_postgres
from poja.database.none import set_database_none
from poja.database.sqlite import set_sqlite
...
def gen(
    ...
    worker_memory=1024,
    worker_batch=5,
    frontal_reserved_concurrent_executions_nb=None,
    worker_reserved_concurrent_executions_nb=None,
    ...
    poja_python_repository_name=None,
    poja_python_repository_domain=None,
    poja_domain_owner=None,
    ...
    if poja_conf is not None:
        with open(poja_conf, "r") as conf_strem:
            ...
            with_queues_nb = int(conf["with_queues_nb"])
            ...
            frontal_reserved_concurrent_executions_nb = (
                int(conf["frontal_reserved_concurrent_executions_nb"])
                if conf["frontal_reserved_concurrent_executions_nb"] != "null"
                else None
            )
            frontal_function_timeout = int(conf["frontal_function_timeout"])
            worker_reserved_concurrent_executions_nb = (
                int(conf["worker_reserved_concurrent_executions_nb"])
                if conf["worker_reserved_concurrent_executions_nb"] != "null"
                else None
            )
            worker_function_1_timeout = int(conf["worker_function_1_timeout"])
            worker_function_2_timeout = int(conf["worker_function_2_timeout"])
            with_snapstart = conf["with_snapstart"]
            aurora_min_capacity = (
                int(conf["aurora_min_capacity"])
                if conf["aurora_min_capacity"] != "null"
                and conf["with_database"] != "none"
                else None
            )
            aurora_max_capacity = (
            ...
            aurora_sleep = (
                int(conf["aurora_sleep"])
                if conf["aurora_sleep"] != "null" and conf["with_database"] != "none"
                else None
            )
            ...
            codeartifact_repository_name = conf["codeartifact_repository_name"]
            codeartifact_domain_name = conf["codeartifact_domain_name"]
            aws_account_id = conf["aws_account_id"]
            prod_db_cluster_timeout = (
                conf["prod_db_cluster_timeout"]
                if conf["prod_db_cluster_timeout"] != "null"
                else 300
            )
            poja_python_repository_name = conf["poja_python_repository_name"]
            poja_python_repository_domain = conf["poja_python_repository_domain"]
            poja_domain_owner = conf["poja_domain_owner"]
            with_file_storage = conf["with_file_storage"]
    ...
    if with_database == "postgres":
        print_warn(
            "Using --with-database=postgres is DEPRECATED. Use --with-database=aurora-postgres instead."
        )

    git_remote_url = (
        f"https://{github_username}:{github_token}@github.com/hei-school/poja.git"
    )
    tmp_dir = mygit.checkout(git_remote_url, GIT_TAG_OR_COMMIT, no_git=True)
    ...
    print_normal("poja_private_repository")
    sed.find_replace(
        tmp_dir,
        "<?poja-python-repository-name>",
        str(poja_python_repository_name),
        exclude,
    )
    sed.find_replace(
        tmp_dir,
        "<?poja-python-repository-domain>",
        str(poja_python_repository_domain),
        exclude,
    )
    sed.find_replace(tmp_dir, "<?poja-domain-owner>", str(poja_domain_owner), exclude)
    print_normal("frontal_reserved_concurrent_executions_nb")
    if frontal_reserved_concurrent_executions_nb is None:
        sed.remove_line_by_keyword(
            f"{tmp_dir}/template.yml",
            "ReservedConcurrentExecutions: <?frontal-reserved-concurrent-executions-nb>",
        )
    else:
        sed.find_replace(
            tmp_dir,
            "<?frontal-reserved-concurrent-executions-nb>",
            str(frontal_reserved_concurrent_executions_nb),
            exclude,
        )
    print_normal("worker_reserved_concurrent_executions_nb")
    if worker_reserved_concurrent_executions_nb is None:
        sed.remove_line_by_keyword(
            f"{tmp_dir}/template.yml",
            "ReservedConcurrentExecutions: <?worker-reserved-concurrent-executions-nb>",
        )
    else:
        sed.find_replace(
            tmp_dir,
            "<?worker-reserved-concurrent-executions-nb>",
            str(worker_reserved_concurrent_executions_nb),
            exclude,
        )
    print_normal("frontal_function_timeout")
    if frontal_function_timeout < 1 or frontal_function_timeout > 900:
        raise Exception(
            "Value for frontal_function_timeout must be between 1 and 900 seconds"
        )
    else:
        sed.find_replace(
            tmp_dir,
            "<?frontal-function-timeout>",
            str(frontal_function_timeout),
            exclude,
        )
    print_normal("worker_function_timeout")
    if worker_function_1_timeout < 1 or worker_function_1_timeout > 900:
        raise Exception(
            "Value for worker_function_1_timeout must be between 1 and 900 seconds"
        )
    else:
        sed.find_replace(
            tmp_dir,
            "<?worker-function-1-timeout>",
            str(worker_function_1_timeout),
            exclude,
        )
        sed.find_replace(
            tmp_dir,
            "<?sqs-1-visibility-timeout>",
            str(worker_function_1_timeout + 1),
            exclude,
        )
    if worker_function_2_timeout < 1 or worker_function_2_timeout > 900:
        raise Exception(
            "Value for worker_function_2_timeout must be between 1 and 900 seconds"
        )
    else:
        sed.find_replace(
            tmp_dir,
            "<?worker-function-2-timeout>",
            str(worker_function_2_timeout),
            exclude,
        )
        sed.find_replace(
            tmp_dir,
            "<?sqs-2-visibility-timeout>",
            str(worker_function_2_timeout + 1),
            exclude,
        )
    if with_file_storage == "false":
        print_normal("with_file_storage")
        set_no_bucket(tmp_dir)
    ...
    set_database_none(with_database, tmp_dir)
    print_normal("with_event_stack")
    set_event_stack(with_queues_nb, tmp_dir, exclude)
    ...
    if with_publish_to_npm_registry == "true":
        ...
        print_normal("codeartifact_repository_name")
        sed.find_replace(
            tmp_dir,
        ...
        print_normal("codeartifact_domain_name")
        sed.find_replace(
            tmp_dir,
    ...
    save_conf(
        ...
        aws_account_id,
        prod_db_cluster_timeout,
        poja_python_repository_name,
        poja_python_repository_domain,
        poja_domain_owner,
...
def save_conf(
    ...
    conf = {
        ...
        "frontal_function_timeout": frontal_function_timeout,
        "worker_memory": worker_memory,
        "worker_function_1_timeout": worker_function_1_timeout,
        "worker_function_2_timeout": worker_function_2_timeout,
        "worker_batch": worker_batch,
        "frontal_reserved_concurrent_executions_nb": frontal_reserved_concurrent_executions_nb,
        "worker_reserved_concurrent_executions_nb": worker_reserved_concurrent_executions_nb,
        ...
        "prod_db_cluster_timeout": str(prod_db_cluster_timeout),
        "poja_python_repository_name": poja_python_repository_name,
        "poja_python_repository_domain": poja_python_repository_domain,
        "poja_domain_owner": poja_domain_owner,
        "with_file_storage": with_file_storage,
        "with_queues_nb": with_queues_nb,
    }
